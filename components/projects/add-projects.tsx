...
import { useAddProjectsMutation, useGetProjectsBySlugQuery, useUpdateProjectsMutation, useUploadFileMutation } from "@/store/handexApi"
...
export function ProjectsForm({ slug }: { slug?: string }) {
    ...
    const [addProjects, { isLoading: projectsLoading, isSuccess: projectsucces }] = useAddProjectsMutation()
    const [updateProjects, { isLoading: projectsUpLoading, isSuccess: projectsUpSucces }] = useUpdateProjectsMutation()
    const { data: projects, isLoading: projectsByIdLoading, isError, error, refetch } = useGetProjectsBySlugQuery(
        {
            slug,
    ...
    useEffect(() => {
        if (slug && projects) {
            form.setValue(`title_${selectedLanguage}` as "title_az" | "title_en" | "title_ru", projects.title);
            form.setValue(`content_${selectedLanguage}` as "content_az" | "content_en" | "content_ru", projects.description);
            form.setValue(`meta_${selectedLanguage}` as "meta_az" | "meta_en" | "meta_ru", projects.meta[0].translations[1].value ? projects.meta[0].translations[1].value : "");
            if (projects.image?.id) {
                form.setValue("featuredImage", projects.image.id);
                setImageState({ ...imageState, preview: projects.image.url || null, id: projects.image.id });
            }
            form.setValue("slug", projects.slug)
        }
    }, [projects, slug]);
    ...
    async function onSubmit(values: z.infer<typeof formSchemaNews>) {
        try {
            ...
            !slug ? await addProjects(postValue).unwrap() : await updateProjects({ params: postValue, id: projects.id }).unwrap()
            toast.success('Layihə uğurla yükləndi')
        } catch (error) {
            toast.error('Layihəni yüklənərkən xəta baş verdi')
        }
