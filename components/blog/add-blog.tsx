...
import { useAddBlogsMutation, useGetBlogsBySlugQuery, useUpdateBlogsMutation, useUploadFileMutation } from "@/store/handexApi"
...
export function BlogsForm({ slug }: { slug?: string }) {
    ...
    const { data: blogs, isLoading: newsByIdLoading, isError, error, refetch } = useGetBlogsBySlugQuery(
        {
            slug,
    ...
    useEffect(() => {
        if (slug && blogs) {
            ...
            form.setValue(`meta_${selectedLanguage}` as "meta_az" | "meta_en" | "meta_ru", blogs.meta[0].translations[1].value ? blogs.meta[0].translations[1].value : "");
            if (blogs.image?.id) {
